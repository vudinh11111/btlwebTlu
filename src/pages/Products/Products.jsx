...
const Products = () => {
	...
	useEffect(() => {
		setDocumentTitle("Sách | Sản phẩm");
	}, []);
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (showFilterDrawer && !event.target.closest(".filters-wrapper")) {
                setShowFilterDrawer(false);
            }
        };
	...
	{loading ? (
		...
		<>
			<div className="filters-wrapper py-1-5 px-1">
				...
				<div className="mobile-filters flex-row flex-align-center flex-justify-between py-0-75 px-1">
					...
					<button
						...
						onClick={() => handleChangeShowFilterDrawer(true)}
					>
			...
			{/*	<div className="grid-row-span">
					<h3 className="main-head text-left py-0-25">Sản phẩm</h3>
				</div>*/}
